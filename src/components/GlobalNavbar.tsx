...
export default function GlobalNavbar({
  ...
  const handleDashboardTabClick = (tab: 'overview' | 'properties' | 'wallet' | 'claim' | 'referrals' | 'profile') => {
    setMobileMenuOpen(false);
    if (!activeUser) {
  ...
  const handleTransactionsTabClick = () => {
    ...
    setTimeout(() => {
      const element = document.getElementById('binance-ledger-module');
      if (element) {
